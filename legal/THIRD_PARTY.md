...
This document lists all third-party components used in SmartOS, their versions, licenses, and copyright information.
...
These dependencies are used for development, testing, and building:
...
All third-party components are compatible with SmartOS's MIT license. The most restrictive license used is GPL-2.0 (pylint), but since pylint is only a development dependency and not distributed with SmartOS, this does not affect the overall license compatibility.
...
This list is automatically generated from the current `requirements.txt` and `pyproject.toml` files. For the most up-to-date information, check the source files in the repository.
